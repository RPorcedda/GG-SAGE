...
def find_node_label_column(
    ...
    for column in node_features_df.columns:
        ...
        if node_feature_labels.is_unique:
            ...
            if edgelist_labels.issubset(node_feature_label_set) or node_feature_label_set.issubset(edgelist_labels):
...
def get_pyg_dataset(
    ...
    features_flag: Optional[bool]=True 
    ) -> Data:
    """
    ...
    if features_flag:
        ...
        x = torch.eye(max([max(edge_index[0]),max(edge_index[1])])+1)


    # Create a directed graph Data object
    directed_data = Data(x=x, edge_index=edge_index)
    print(f"{dataset} dataset preprocessed")
    print(f'Number of nodes: {directed_data.num_nodes}')
    print(f'Number of edges: {directed_data.num_edges}')
    print(f'Features matrix: {directed_data.x.shape}. Features Flag:{features_flag} ')
    print()
