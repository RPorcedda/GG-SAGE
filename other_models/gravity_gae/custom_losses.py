...
def recon_loss(logits, ground_truths, EPS = 1e-10):
    if torch.isnan(logits.x).any() or torch.isnan(ground_truths).any():
        print("NaNs detected in logits or ground_truths")
        return 1.
...
def auc_loss(logits, ground_truths):
    if torch.isnan(logits.x).any():
        print("NaNs detected in logits")
        return 0.
    if torch.isnan(ground_truths).any():
        print("NaNs detected in ground_truths")
        return 0.
